...
export default function Header() {
  ...
  return (
    <header className="flex items-center justify-between border-b border-white/10 bg-black px-4 md:px-6 py-4 text-white">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <span className="hidden md:block h-10 w-[1px] bg-white/10" />

        <button
          onClick={handleLogout}
          className="rounded-full border border-white/10 bg-[#11144C] px-3 md:px-4 py-2 text-sm text-white hover:bg-white/10"
        >
          {t('Logout', 'התנתק')}
        </button>
        <div className="hidden md:flex rounded-full bg-white/10 px-4 py-2 text-sm">JD</div>
      </div>
    </header>
  );
}
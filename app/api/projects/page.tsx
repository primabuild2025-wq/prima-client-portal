...
export default function ProjectsPage() {
  ...
  // Re-translate whenever language or projects change
  useEffect(() => {
    if (projects.length === 0) { setTranslatedProjects([]); return; }
    applyTranslations(projects);
  }, [lang, projects]);

  const applyTranslations = async (data: any[]) => {
    setTranslating(true);
    try {
      const texts = data.flatMap(p => [p.name || '', p.description || '']);
      const translated = await translateContent(texts);
      setTranslatedProjects(data.map((p, i) => ({
        ...p,
        name:        translated[i * 2]        || p.name,
  ...
  const displayed = translatedProjects.length > 0 ? translatedProjects : projects;
